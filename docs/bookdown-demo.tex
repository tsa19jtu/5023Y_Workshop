...
\hypertarget{using-shapes}{%
\subsection{Using shapes}\label{using-shapes}}

Using colours to tell categories apart can be useful, but as we can see in the example above, not always completely accessible.

A complementary approach is to set the \emph{shape} of your geoms according to categorical variables.
...
\NormalTok{penguins }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\NormalTok{flipper\_length\_mm, }
             \AttributeTok{y =}\NormalTok{ body\_mass\_g))}\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{colour=}\NormalTok{species,}
                 \AttributeTok{shape=}\NormalTok{species))}\SpecialCharTok{+} \CommentTok{\# as with colours we could set these to specific values if we want}
  \FunctionTok{scale\_color\_manual}\NormalTok{(}\AttributeTok{values=}\NormalTok{penguin\_colours)}\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\includegraphics[width=0.8\linewidth]{images/shapes}
...
\hypertarget{reports-with-rmarkdown---week-four}{%
\chapter{Reports with Rmarkdown - Week Four}\label{reports-with-rmarkdown---week-four}}
...
When typed in-line within a section of what would otherwise be Markdown text, it knows to produce an r output instead 2021-10-21
...
\item
  ...
  \item
    Write a summary of the results where the figures and table are referenced but you \emph{must} describe the results too. e.g.~results shown in Figure 1 is \textbf{not ok}, \emph{in your own words} describe the direction and effect size of any differences and then refer to Figures and tables
  \end{itemize}
\item
