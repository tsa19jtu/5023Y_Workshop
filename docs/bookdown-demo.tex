...
\begin{figure}
\includegraphics[width=0.8\linewidth]{images/penguin_github} \caption{Click on the copy raw contents button}\label{fig:unnamed-chunk-46}
\end{figure}
...
Make sure you type these \textbf{exactly} as printed here - remember that to R `data' and `Data' are two different things!
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{images/project} \caption{An example of a typical R project set-up}\label{fig:unnamed-chunk-47}
\end{figure}
...
\begin{quote}
\texttt{view()} lets you do cool stuff like reorder rows and quickly scroll through the dataset without affecting the underlying data.
\end{quote}
...
What figures might we want to produce from our data?
...
\item
  How many adults produced a successful clutch?
\end{itemize}
...
\hypertarget{numbers-sex-and-bodyweight-of-penguins}{%
\section{Numbers, sex and bodyweight of penguins}\label{numbers-sex-and-bodyweight-of-penguins}}
...
\begin{quote}
You have just had a crash course in using the \texttt{pipe} and \texttt{dplyr} to produce quick data summaries. Have a go at making some other summaries of your data, perhaps the numbers of penguins by island or region. Try some combinations.
\end{quote}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{images/stacked_bar} \caption{A first insight the number of male and female penguins of each species in our study}\label{fig:unnamed-chunk-77}
\end{figure}
...
Write and run a script, with appropriate comments, that produces a summary tibble and graph for the number of penguins of each species, on the three study islands
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{images/Distribution} \caption{Frequency distribution of culmen length in penguins}\label{fig:unnamed-chunk-79}
\end{figure}

\begin{quote}
Have a go at changing the value specified to the bins argument, and observe how the figure changes.
\end{quote}
...
\item
  ...
  \item
    Cons: very condition dependent, likely to change over the year
  \end{itemize}
\end{itemize}

Let's take a look at the distribution of flipper length among our three species.
...
\NormalTok{penguins }\SpecialCharTok{\%\textgreater{}\%} 
  ...
  \FunctionTok{cor\_test}\NormalTok{(flipper\_length\_mm, culmen\_length\_mm) }\CommentTok{\# correlation between body mass and bill length}
\end{Highlighting}
\end{Shaded}

We can see that these two variables `co-vary' a lot, but this appears to be quite species specific. We can already make the insight that Adelie penguins appear to have the shortest bill length relative to flipper length.
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{images/bill_length_distribution} \caption{Frequency distribution of relative bill length in three species of penguins, Adelie, Chinstrap and Gentoo}\label{fig:unnamed-chunk-89}
\end{figure}
...
Looking at these distributions, how do you think the mean \& median values will compare in these three species? Think about it first - then run this code.
...
\tightlist
\item
  Repeat these steps for the variable \texttt{relative\_bill\_depth}, \texttt{delta\_15n} and \texttt{delta\_13c} - add all appropriate comments and commands to your R Script.
\end{itemize}
...
Note - we will do a deeper dive into ggplot later.
\end{quote}

Try producing these figures for the \texttt{culmen\_depth\_mm}, \texttt{delta\_15n} and \texttt{delta\_13c} variables as well. Make notes of the relative differences.
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{images/summary} \caption{Difference in relative bill length and depth, and heavy carbon and nitrogen ratios, in male and female penguins from three different species - Adelie, Chinstrap and Gentoo}\label{fig:unnamed-chunk-96}
\end{figure}
...
\item
  Complete this week's quiz on Blackboard
\end{itemize}
