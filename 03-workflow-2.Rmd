...
## Numbers, sex and bodyweight of penguins
...
> You have just had a crash course in using the `pipe` and `dplyr` to produce quick data summaries. Have a go at making some other summaries of your data, perhaps the numbers of penguins by island or region. Try some combinations. 
...
penguins %>% 
  ...
  cor_test(flipper_length_mm, culmen_length_mm) # correlation between body mass and bill length
```
...
* Repeat these steps for the variable `relative_bill_depth`, `delta_15n` and `delta_13c` - add all appropriate comments and commands to your R Script. 
...
Try producing these figures for the `culmen_depth_mm`, `delta_15n` and `delta_13c` variables as well. Make notes of the relative differences. 
